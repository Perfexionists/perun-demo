...
double switchCost(const std::string& s1, const std::string& s2) {
...
double computeTransformationCost(const std::vector<std::string>& trace1, const std::vector<std::string>& trace2, int i, int j) {
    ...
    if (trace1[i] == trace2[j]) {
        ...
        double switchInstructions = switchCost(trace1[i], trace2[j]) + computeTransformationCost(trace1, trace2, i + 1, j + 1);
